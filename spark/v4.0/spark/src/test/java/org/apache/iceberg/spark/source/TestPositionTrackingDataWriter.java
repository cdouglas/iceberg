...
public class TestPositionTrackingDataWriter {
  ...
  @Test
  public void testExtractMetadataAndProject() throws IOException {
    ...
    when(mockRow.get(0, null)).thenReturn(1);
    when(mockRow.get(1, null)).thenReturn(UTF8String.fromString("test"));
    when(mockRow.get(2, null)).thenReturn(100L);

    writer.write(mockRow);

    // Verify delegate was called with projected row (without metadata)
    ...
    InternalRow projectedRow = rowCaptor.getValue();
    assertThat(projectedRow.numFields()).isEqualTo(3); // Only data columns
  ...
  public void testMultipleWrites() throws IOException {
    ...
    // Write 3 rows from same source
    for (int i = 0; i < 3; i++) {
      InternalRow mockRow = mock(InternalRow.class);
      when(mockRow.getUTF8String(3)).thenReturn(UTF8String.fromString("source.parquet"));
      when(mockRow.getLong(4)).thenReturn((long) i);
      when(mockRow.isNullAt(any(Integer.class))).thenReturn(false);
      when(mockRow.get(0, null)).thenReturn(i);
      when(mockRow.get(1, null)).thenReturn(UTF8String.fromString("data" + i));
      when(mockRow.get(2, null)).thenReturn((long) i * 10);
  ...
  @Test
  public void testProjectionWithNulls() throws IOException {
    PositionTrackingDataWriter writer =
        new PositionTrackingDataWriter(mockDelegate, mockTable, fileSetId, dsSchema);
    ...
    InternalRow projectedRow = rowCaptor.getValue();
    assertThat(projectedRow.numFields()).isEqualTo(3);
  }
  ...
  public void testMultipleSourceFiles() throws IOException {
    ...
    for (String sourceFile : sourceFiles) {
      for (int i = 0; i < 10; i++) {
        ...
        when(mockRow.getLong(4)).thenReturn((long) i);
        when(mockRow.isNullAt(any(Integer.class))).thenReturn(false);
        when(mockRow.get(any(Integer.class), any())).thenReturn(i);
