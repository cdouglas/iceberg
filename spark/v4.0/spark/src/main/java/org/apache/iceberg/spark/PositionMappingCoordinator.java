...
public class PositionMappingCoordinator {
  ...
  private Map<String, RewriteFileGroup.FilePositionMapping> aggregateMappings(
    ...
    for (Map.Entry<String, List<PositionMapping>> entry : rawMappings.entrySet()) {
      ...
      // Identify runs by detecting gaps in source OR target positions
      ...
      String currentTargetFile = first.targetFile;
      long runLength = 1;
        ...
        if (sourceConsecutive && targetConsecutive && sameTarget) {
          ...
          runs.add(
              new RewriteFileGroup.FilePositionMapping.Run(
                  runStartSource, runStartTarget, runLength));

          LOG.debug(
              "Detected gap or file change at source position {} -> {} in {}",
              prev.sourcePos,
              curr.sourcePos,
              sourceFile);
          ...
          currentTargetFile = curr.targetFile;
          runLength = 1;
        }
      }
      ...
      // Add final run
      runs.add(
          new RewriteFileGroup.FilePositionMapping.Run(runStartSource, runStartTarget, runLength));

      LOG.debug(
          "Aggregated {} position mappings into {} runs for source file {}",
          positions.size(),
          runs.size(),
          sourceFile);
