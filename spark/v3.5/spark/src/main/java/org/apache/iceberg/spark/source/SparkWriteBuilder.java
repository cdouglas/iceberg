...
class SparkWriteBuilder implements WriteBuilder, SupportsDynamicOverwrite, SupportsOverwrite {
  ...
  public Write build() {
    ...
    //
    // However, when position tracking is enabled, the DataFrame contains _file and _pos
    // metadata columns instead of row lineage columns (normal scans don't populate row
    // lineage). Row lineage columns are still written to files (generated by the writer),
    // but they should not be added to sparkWriteSchema since they don't exist in the
    // input DataFrame.
    ...
    if (writeRequiresPositionTracking && !writeAlreadyIncludesFilePos) {
      ...
      sparkWriteSchema =
          sparkWriteSchema.add(MetadataColumns.ROW_POSITION.name(), LongType$.MODULE$);
    }
