#!/usr/bin/env bash
# Wrapper for RunMain that supplies the --add-opens flags Spark 3.5 needs on Java 17/21.
# Usage:
#   scripts/run.sh --state-tar /path/to/baseline-K1k.tar \
#                  --working-dir /tmp/run-work \
#                  --variant baseline --k 1000 \
#                  --iterations 3 --output /tmp/run-work/results.jsonl
#
...
# Same JVM args as setup.sh (Spark 3.5 reflects into many JDK internals on Java 17/21).
JVM_OPTS=(
...
exec java "${JVM_OPTS[@]}" -cp "$JAR" \
  org.apache.iceberg.benchmark.compaction.RunMain "$@"
