...
public class ReadTableAction implements LogAction {
  private final int tblId;
  private final int tblVersion;
  private final TableIdentifier tableIdentifier;

  public ReadTableAction(int tblId, int tblVersion, TableIdentifier tableIdentifier) {
    this.tblId = tblId;
  ...
  public void apply(CatalogState.Builder builder) {
    // Read operations don't modify state, but can be used for conflict detection
    // No state changes needed
  }

  @Override
  public void serialize(DataOutputStream dos) throws IOException {
    dos.writeByte(Type.READ_TABLE.ordinal());
    dos.writeInt(tblId);
  ...
  public static ReadTableAction deserialize(DataInputStream dis) throws IOException {
    int tblId = dis.readInt();
    int tblVersion = dis.readInt();
    // Note: TableIdentifier needs to be resolved during application
    return new ReadTableAction(tblId, tblVersion, null);
  }
